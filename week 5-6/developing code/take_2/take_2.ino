...
void loop() {
  update_gains (); // it increament and deacreaments gain as per comanded (in testing_print.h)
  ...
  float bot_inclination = dataFusion(0.98); //it communicates with mpu derives data for gyro and accel and fuses them to gather
                                            //returns filterd inclination of bot
                                            //argument to be pass = filter coefficient of complementry filter default is 0.97
  //######################################################---------------------------------------------PID controler here
  ...
  error_d = (error - prev_error) / Ts;      //it diffrenciets error 
  prev_error = error;                       // keeping trace back of previous error 

  Rmotor_torque = (Kp * error) + (Ki * error_i) + (Kd * error_d);     // PID algorythem 
  run_Rwheel(Rmotor_torque);                                          //it runs reaction wheel motor according to PID calculation
                                                                      // pass the argument of PID calculation 

  //######################################################
  print_data(pitch, error,5);                //the function is to monitor parameters just pass the perameter as argument (in testing_print.h)(max 5 argument is allwed)
  //######################################################
  //the while loop burns exesive time to set uniform sampling for defined in Ts varieble
...
//                    it prevents to windup integrator towards infinity for constunt error because of machine saturation 
//############################################################
...
void integrate_error()
{
  ...
  {
    error_i = error_i + error * Ts;             // integrate normally if "Ki*error_i" valu is within the range of saturation(-255 to 255)
  }
  else if (Ki * error_i > 255)
  {
    error_i = float(int(255 / Ki));            // if "Ki*error_i" value is exeding 255 just turn of integrator
  }
  else if ( Ki * error_i < -255)
  {
    error_i = float(int(-255 / Ki));          //if "Ki*error_i" value becoming lower than -255 just turn of integrator 
  }
}
